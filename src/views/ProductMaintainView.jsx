...
const ProductMaintainView = () => {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center h-64 gap-3">
            ...
            <span className="text-slate-500 text-[11px] font-black uppercase tracking-widest">正在同步商品数据...</span>
        </div>
    );
    ...
    return (
        <div className="h-full overflow-y-auto p-4 md:p-10 space-y-6 md:space-y-10 animate-in fade-in slide-in-from-bottom-8 zoom-in-95 duration-700">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h3 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight whitespace-nowrap">商品维护</h3>
                    <p className="text-slate-500 text-[11px] font-bold uppercase mt-1 whitespace-nowrap tracking-wider">Product Catalog Intelligence</p>
                </div>
            ...
            <div className="grid grid-cols-4 gap-3">
                {[
                    ...
                    <div key={i} className={`rounded-2xl border px-4 py-3 flex items-center gap-3 ${
                        ...
                        <div>
                            <p className="text-[11px] text-slate-500 font-black uppercase tracking-widest">{k.label}</p>
            ...
            <div className="solid-card rounded-[24px] border border-slate-200 overflow-hidden bg-white">
                <div className="px-6 py-4 bg-slate-50/60 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-[11px] text-slate-500">显示最近同步的 {products?.length} 件商品</span>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/30 border-b border-slate-100">
                                <th className="px-6 py-4 text-[11px] font-black text-slate-500 uppercase tracking-widest">商品信息</th>
                                <th className="px-6 py-4 text-[11px] font-black text-slate-500 uppercase tracking-widest">站点</th>
                                <th className="px-6 py-4 text-[11px] font-black text-slate-500 uppercase tracking-widest">价格</th>
                                <th className="px-6 py-4 text-[11px] font-black text-slate-500 uppercase tracking-widest">状态</th>
                                <th className="px-6 py-4 text-[11px] font-black text-slate-500 uppercase tracking-widest">健康分</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr key={idx} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[11px] text-slate-500 font-mono mt-0.5">{p.item_id}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded-md text-[11px] font-black uppercase">{p.site_id}</span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className="text-[11px] font-black text-slate-700">${p.price}</span>
                                    </td>
                                    <td className="px-6 py-4">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[11px] font-black text-slate-600">{p.health_score}</span>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                ...
                <div className="px-6 py-4 bg-slate-50/30 text-center">
                    <p className="text-[11px] text-slate-500 font-bold uppercase tracking-widest">已加载前 10 条数据 · 完整操作请前往「商品性能表」</p>
                </div>
            </div>
        </div>
    );
